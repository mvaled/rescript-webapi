...
let test_arrayBuffer = blob =>
  blob
  ->arrayBuffer
  ->Js.Promise2.then(buffer =>
    buffer->Js.Typed_array.ArrayBuffer.byteLength->Js.log->Js.Promise2.resolve
  )
...
let test_text = blob => blob->text->Js.Promise2.then(string => string->Js.log->Js.Promise2.resolve)
...
let test_blobPart_arrayBuffer: t => Js.Promise2.t<blobPart> = blob =>
  blob
  ->arrayBuffer
  ->Js.Promise2.then(buffer => buffer->arrayBufferToBlobPart->Js.Promise2.resolve)
...
let test_blobPart_dataView: t => Js.Promise2.t<blobPart> = blob =>
  blob
  ->arrayBuffer
  ->Js.Promise2.then(buffer =>
    dataViewToBlobPart(Js.Typed_array.DataView.make(buffer))->Js.Promise2.resolve
  )
